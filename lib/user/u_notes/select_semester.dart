...
import 'package:college_app/user/u_notes/usersubject_selection.dart';
import 'package:college_app/user/user_home.dart';
import 'dart:math' as math;

class SelectSemesterUserPage extends StatefulWidget {
  final String selectedCollege;
  final String branch;

  const SelectSemesterUserPage({
    super.key, 
  ...
  @override
  State<SelectSemesterUserPage> createState() => _SelectSemesterUserPageState();
}

class _SelectSemesterUserPageState extends State<SelectSemesterUserPage> 
  ...
  List<Map<String, dynamic>> _getSemesterData() {
    final int semesterCount = widget.branch == 'Basic Science' ? 2 : 6;
    final List<Map<String, dynamic>> semesters = [];
    
    for (int i = 0; i < semesterCount; i++) {
      final semesterNumber = widget.branch == 'Basic Science' ? i + 1 : i + 3;
      semesters.add({
        'title': 'Semester $semesterNumber',
        'subtitle': 'View subjects and materials',
        'icon': Icons.school,
                ...
                children: [
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemCount: _getSemesterData().length,
                      itemBuilder: (context, index) {
                        final semester = _getSemesterData()[index];
                        return CustomSemester(
                          title: semester['title'],
                          subtitle: semester['subtitle'],
                          icon: semester['icon'],
                          color: semester['color'],
                          onTap: () => _navigateToSubjectsPage(
                            context, 
                            semester['title']
                          ),
                          index: index,
                          totalItems: _getSemesterData().length,
  ...
  void _navigateToSubjectsPage(BuildContext context, String semester) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => UserSubjectSelectionPage(
...
class CustomSemester extends StatelessWidget {
  final String title;
