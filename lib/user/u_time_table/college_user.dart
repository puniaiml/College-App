...
class CollegeUserPage extends StatefulWidget {
  const CollegeUserPage({super.key});

  @override
  _CollegeUserPageState createState() => _CollegeUserPageState();
}

class _CollegeUserPageState extends State<CollegeUserPage> {
  ...
  void initState() {
    ...
    _loadSelectedCollege(); // Load previously selected college
  }

  Future<void> _loadSelectedCollege() async {
    final prefs = await SharedPreferences.getInstance();
    setState(() {
      _selectedCollege = prefs.getString('selectedCollege'); // Get saved college
    });
  }

  Future<void> _saveSelectedCollege(String college) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('selectedCollege', college); // Save selected college
  }
  ...
  Future<void> _navigateToBranchSelection() async {
    if (_selectedCollege != null) {
      ...
      Get.to(() => BranchUser(selectedCollege: _selectedCollege!));
    } else {
                    ...
                    return _CollegeItem(
                      ...
                      onTap: () {
                        setState(() {
                          ...
                          _saveSelectedCollege(college['name']); // Save the selected college
                        });
                      },
                    );
                  },
